...
if [ x"${PROJECT_CLEAN}" == x"true" ];then
	rm -rf ${DIR_ROOT}/build/ 2>&1 > /dev/null
...
mkdir -pv ${DIR_ROOT}/build/ > /dev/null 2>&1
...
if [ x"${BUILD_LIBS}" == x"true" ];then
    rm -rf ${DIR_ROOT}/build > /dev/null 2>&1
    conan install . -of=${DIR_ROOT}/build --build=missing -s build_type=${BUILD_TYPE}
fi
