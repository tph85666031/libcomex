...
public:
    ...
    WaterMark& setColor(uint32 color);//颜色
    WaterMark& setBackgroundColor(uint32 color);//背景颜色
    ...
    WaterMark& setBinarize(uint8 binarize = 100); //水印像素二值化
    ...
    static bool SetPNGAlphaValue(const char* file_to, const char* file_from, int value,
                                 uint8 threshold_begin = 1, uint8 threshold_end = 255);
    //设置图片亮度,取值范围为[0,1]
...
private:
    ...
    uint32 color = 0xC0C0C000;//默认25%灰
    uint32 color_background = 0xFFFFFF00;//纯白
    int width = 100;
    int space_x = 0;
    int space_y = 0;
    int angle = 0;
